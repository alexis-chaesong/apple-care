...
class GetKeyword(Node):
    ...
    def extract_keyword(self, output_message):  # d2 이 함수 일부 수정함
        # 명확한 도구명은 LLM 응답에 의존하지 않고 검출 모델의 클래스명으로 변환한다.
        aliases = {
            "망치": "hammer",
            "해머": "hammer",
            "hammer": "hammer",
        ...
        lowered_message = output_message.lower()
        direct_matches = [
            (lowered_message.find(alias), tool)
            for alias, tool in aliases.items()
            if alias in lowered_message
        ]
        if direct_matches:
            direct_matches.sort()
            tools = list(dict.fromkeys(tool for _, tool in direct_matches))
            self.get_logger().info(f"Direct tool mapping: {tools}")
            return tools

        response = self.lang_chain.invoke({"user_input": output_message})
        result = response.content

        object_text, target_text = result.strip().split("/", 1)
        objects = object_text.split()
        targets = target_text.split()

        print(f"llm.s response: {objects}")
    ...
    def get_keyword(self, request, response):  # 요청과 응답 객체를 받아야 함    # d2 이 함수 일부 수정함
        ...
        # Wake-word 스트림을 닫고 STT가 같은 마이크를 사용하도록 한다.
...
def main():  # d2 메인문 일부 수정
    ...
    # robot_control의 서비스 요청으로 인식한 결과가 로봇까지 전달되도록 한다.
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()
