...
from obj_detection.yolo import MIN_KNOWN_CONFIDENCE
...
class DebugOverlayMixin:
    ...
    def _draw_all_detections(self, colormap, depth_frame, detections):
        ...
        것들 중 depth 실측 지름이 최근 정상 사과 평균보다 확연히 작으면 여기서도
        "apple_small"로 재분류될지 미리 보여준다 (실제 판정은 handle_get_status의
        `_classify_size`가 하고, 여기는 같은 기준의 미리보기).
