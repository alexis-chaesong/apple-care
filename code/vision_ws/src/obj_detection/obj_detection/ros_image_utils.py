...
처음엔 이 분기를 `ROS_DISTRO == "humble"` 기준으로 했었는데, 이건 실제
실패 조건("numpy가 2.x인가")의 부정확한 대리 지표였다:
    - numpy2가 안 깔린 Humble 환경도 무조건 cv_bridge를 우회하게 됨 (불필요)
    - 반대로 Jazzy 등 다른 배포판에서 pip으로 numpy2를 깔았는데 그 환경의
      cv_bridge가 numpy1 ABI로 빌드돼 있는 조합이면 못 잡아냄 (놓침)
      (실제로 이 프로젝트에서도 동일한 코드가 배포판이 아니라 "각자 머신에
      pip으로 numpy2가 얹혔는지"에 따라 사람마다 다르게 재현/미재현됐다.)
그래서 실제 실패 조건에 가장 가까운 신호인 **"지금 import된 numpy가
메이저 2 이상인가"**로 직접 분기하도록 바꿨다. numpy 2.0 출시 이전에 빌드된
cv_bridge는 전부 numpy1 ABI이므로, 이 체크 하나로 두 실패 모드를 다 잡는다.
