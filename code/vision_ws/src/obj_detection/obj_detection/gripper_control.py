...
OnRobot RG2 그리퍼를 Modbus/TCP로 직접 제어하는 모듈 (Compute Box에 직접 접속).
prepare_camera.py에서 카메라 노드를 켜기 전에 그리퍼를 여는 용도로 씀.

apple_care_robot(robot_ws)/openclose.py와 로직이 동일함 - obj_detection(vision_ws)이
robot_ws 패키지를 import하지 않고도 단독으로 동작하도록 이 파일 안에 그대로
복제해서 둠. 그리퍼 제어 로직 자체를 수정할 일이 생기면 두 파일 다 같이
고쳐야 함 (apple_care_robot/openclose.py도 참고).
...
def _send_command(force: int, width: int) -> bool:
    ...
    if not client.connect():
        print(f"[gripper_control] OnRobot 그리퍼({GRIPPER_IP}:{GRIPPER_PORT})에 연결하지 못했습니다!")
        return False
