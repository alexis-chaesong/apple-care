...
async def call_gpt4o_vlm(image_bytes: bytes) -> dict:
    ...
    system_prompt = (
        ...
        "이 이미지에는 이미 이 시스템이 사과로 인식하고 처리 중인 물체와, "
        "아직 무엇인지 식별되지 않은 새로운 물체 하나가 함께 보일 수 있다. "
        "너의 임무는 오직 그 '아직 식별되지 않은 새로운 물체 하나'가 정확히 "
        "무엇인지 한국어 명사(구) 단 하나로 간결하게 답하는 것이다. "
        "이미 사과로 보이는 물체는 답에서 절대 언급하지 마라 - 여러 물체를 "
        "나열하거나 쉼표로 여러 개를 답하지 마라. 이 물체를 어디로 보내야 "
        "하는지, 어떻게 처리해야 하는지는 절대 판단하지 마라 - 오직 새 물체가 "
        "무엇인지만 답한다. "
        '{"identified_object": string} 형식의 JSON으로만 출력해라. '
        "새로운 물체를 전혀 특정할 수 없으면 identified_object를 null로 출력해라."
    )
    user_prompt = "이 이미지에서 아직 식별되지 않은 새로운 물체 하나는 무엇인가?"
