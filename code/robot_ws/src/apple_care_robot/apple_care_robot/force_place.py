...
#
# 실측으로 확인된 문제: 잡고 있는 상태에서 접촉 감지가 여유(2.5초) 안에 못 끝나서
# "접촉 실패"로 처리되어 그립을 유지한 채 작업대로 재배치되는 사례(사실상 놓기
# 실패)가 있었음. 여유분을 2초 더 늘림(2.5 -> 4.5). 그래도 반복되면 아래 대안도
# 고려할 것:
#   1) FORCE_THRESHOLD(4N)가 너무 높아 접촉을 늦게 인정하는 것일 수도 있음 -
#      낮춰서(예: 3~3.5) 더 빨리 인정되게 하는 방법도 있음.
#   2) STALL_TIME_SEC(2.0초)이 이 여유 안에서 한 번도 만족 못 하는 경우일 수도
#      있음 - 로그의 '[디버그] 현재 Fz=...' 출력을 보고 타임아웃 직전까지 z가
